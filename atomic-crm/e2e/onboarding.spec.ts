...
test("user onboarding", async ({ page, isMobile, menu, dismissToast }) => {
  ...
  await expect(page).toHaveTitle(/Chaster CRM/);
  await expect(page.getByText("Welcome to Chaster CRM")).toBeVisible();
  ...
  await expect(page.getByText("Install Chaster CRM")).toBeVisible();
