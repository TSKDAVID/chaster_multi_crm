...
import { getSupabaseBrowserEnv, getSupabaseClient } from "./supabase";
...
const getDataProviderWithCustomMethods = () => {
  ...
  return {
    ...
    async salesCreate(body: SalesFormData) {
      ...
      return data.invite_email_sent === true
        ? { ...sale, invite_email_sent: true as const }
        : sale;
    },
    async delete(resource: string, params: DeleteParams) {
      if (resource === "sales") {
        ...
        // POST + JSON body: reliable (DELETE/query can be dropped by gateways). Edge: action "delete_user".
        ...
        if (error || !data?.ok) {
          const errorDetails = await (async () => {
            try {
              return (await (error as { context?: Response })?.context?.json()) ??
                {};
            } catch {
              return {};
            }
    ...
    async updatePassword(id: Identifier) {
      ...
      if (userError || !user?.email) {
        console.error("update_password.user_error", userError);
        throw new Error("Failed to resolve authenticated user email");
      }

      const redirectTo = `${window.location.origin}/#/set-password`;
      const { data: passwordUpdated, error } = await supabase.auth.resetPasswordForEmail(
        user.email,
        { redirectTo },
      );
