...
import { hasSupabaseAuthSuccessUrlPayload } from "../../auth/supabaseAuthUrl";
...
/** After invite/set-password, drop cached sale row so identity refetches. */
export function invalidateCrmAuthIdentityCache() {
  clearCache();
}
  ...
  return {
    ...
    checkAuth: async (params) => {
      ...
      const isPublicCheckoutPath =
        ...
        pathname === "/checkout/test/success" ||
        hash.includes("#/landing-test") ||
      ...
      if (pathname === "/auth/invite-error") {
        return;
      }

      // Invite / recovery success: hash or query still carries tokens from Supabase verify redirect.
      // Do not run sign-out / sign-up redirects until the SPA has consumed them (e.g. /set-password).
      // Error hashes (#error=…) are handled separately; do not bypass checkAuth for those.
      if (hasSupabaseAuthSuccessUrlPayload()) {
        return;
      }

      // Invite / recovery links carry tokens in the URL; Supabase auth must redirect to
      // set-password before we force "create first user" sign-up on an uninitialized project.
      try {
        ...
        if (
          ...
          typeof e === "object" &&
          "redirectTo" in e &&
          (e as { redirectTo?: unknown }).redirectTo !== undefined
        ) {
          throw e;
        }
      }
