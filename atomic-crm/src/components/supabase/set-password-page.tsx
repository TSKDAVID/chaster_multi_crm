...
import { resolveChasterPostAuthPath } from "@/components/atomic-crm/auth/chasterPostAuthPath";
import { invalidateCrmAuthIdentityCache } from "@/components/atomic-crm/providers/supabase/authProvider";
...
export const SetPasswordPage = () => {
  ...
  const submit = async (values: FormData) => {
    try {
      ...
      notify("crm.auth.password_set_success", { type: "success" });
      invalidateCrmAuthIdentityCache();
      await queryClient.invalidateQueries({ queryKey: ["chaster-access"] });
      const dest = await resolveChasterPostAuthPath();
