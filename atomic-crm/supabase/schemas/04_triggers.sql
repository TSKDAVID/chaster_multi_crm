...
-- Auto-populate tenant_id from current user's membership on insert
create or replace trigger set_company_tenant_id_trigger
    before insert on public.companies
    for each row execute function public.set_tenant_id_default();

create or replace trigger set_contact_tenant_id_trigger
    before insert on public.contacts
    for each row execute function public.set_tenant_id_default();

create or replace trigger set_deal_tenant_id_trigger
    before insert on public.deals
    for each row execute function public.set_tenant_id_default();

create or replace trigger set_task_tenant_id_trigger
    before insert on public.tasks
    for each row execute function public.set_tenant_id_default();

create or replace trigger set_contact_notes_tenant_id_trigger
    before insert on public.contact_notes
    for each row execute function public.set_tenant_id_default();

create or replace trigger set_deal_notes_tenant_id_trigger
    before insert on public.deal_notes
    for each row execute function public.set_tenant_id_default();

create or replace trigger set_tags_tenant_id_trigger
