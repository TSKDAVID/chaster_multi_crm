...
description: This document explains how to deploy Chaster CRM to production.
---

## Deploying The Backend

The entire backend of Chaster CRM is hosted on Supabase. The backend is composed of a Postgres database, a REST API, and edge functions. Check out the [Supabase Configuration](../supabase-configuration) section for details.
...
Chaster CRM contains GitHub actions for continuous integration and delivery. To enable these actions, you will
...
> **Note:** The `POSTMARK_*` and `VITE_INBOUND_EMAIL` variables are required for Chaster CRM's inbound email features. Have a look at the the [inbound email configuration](../inbound-email-configuration) to learn more about their usage and setup.
