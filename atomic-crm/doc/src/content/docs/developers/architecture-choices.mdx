...
description: This document explains some of the architecture decisions made in the development of Chaster CRM.
---

This document explains some of the architecture decisions made in the development of Chaster CRM.
...
For the backend, Chaster CRM leverages [Supabase](https://supabase.com/). Supabase provides a PostgreSQL database, a REST API, authentication, storage, and edge functions. In practice, this means that there is very little backend code to maintain (a few edge functions for user management and inbound email processing).

## Mutable Dependencies

The Chaster CRM code primarily lives in the `src/components/atomic-crm` directory. With less than 15,000 lines of code, it is a small codebase that can be easily customized.

The Chaster CRM code also contains some mutable dependencies, i.e. libraries that are directly integrated into the codebase to be modified if needed. These dependencies are located in the following directories:
...
Some pages in Chaster CRM require data from multiple tables. To simplify the frontend code and reduce the HTTP overhead, Chaster CRM uses database views to abstract the complexity of the queries.
...
User credentials are stored in Supabase's `auth.users` table. Supabase does not allow to add columns to this table. That's why additional user details are stored in a `sales` table created by Chaster CRM. A database trigger is used to automatically sync the `sales` record when a user is created or updated (e.g. for the `first_name` and `last_name` fields).
...
To solve this problem, Chaster CRM uses a `users` edge function in charge of:

- Verifying the current user's permissions
- Creating and updating users

Chaster CRM does not support user deletion to avoid data losses. Yet, it is possible to disable a user's account (relying on Supabase's ban feature).
