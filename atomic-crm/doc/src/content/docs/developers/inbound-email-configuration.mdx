...
description: This document explains how to configure inbound email processing in Chaster CRM.
---
...
Chaster CRM uses [Postmark](https://postmarkapp.com/) to receive and process inbound emails.
...
Chaster CRM uses a Supabase Edge Function to handle the webhook and process the received emails. You need to configure your Supabase project to handle the incoming requests.
...
This is necessary because it is used both client-side, to display the Inbound Email in the Chaster CRM app, and server-side (by the Edge Function) to detect when an email is forwarded to Chaster CRM.
:::
